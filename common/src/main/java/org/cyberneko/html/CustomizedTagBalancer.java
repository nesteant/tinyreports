 ...
 *         Customized balancer. Original copied from {@link HTMLTagBalancer}
 */
public class CustomizedTagBalancer implements XMLDocumentFilter, HTMLComponent {
	...
	private static final int[] RECOGNIZED_HEAD_SPECIALS = {
			ExtendedHtmlElements.BOOKMARKS,
			ExtendedHtmlElements.BOOKMARK
	};
	...
	public void startDocument(XMLLocator locator, String encoding,
		...
		if (fragmentContextStack_ != null) {
			...
			for (int i = 0; i < fragmentContextStack_.length; ++i) {
				...
				final ExtendedHtmlElements.Element elt = ExtendedHtmlElements.getElement(name.localpart);
	...
	public void startElement(final QName elem, XMLAttributes attrs, final Augmentations augs)
		...
		final ExtendedHtmlElements.Element element = getElement(elem);
		final short elementCode = element.code;

		// the creation of some elements like TABLE or SELECT can't be forced. Any others?
		if (isForcedCreation && (elementCode == ExtendedHtmlElements.TABLE || elementCode == ExtendedHtmlElements.SELECT)) {
			return; // don't accept creation
		}

		// ignore multiple html, head, body elements
		if (fSeenRootElement && elementCode == ExtendedHtmlElements.HTML) {
			notifyDiscardedStartElement(elem, attrs, augs);
			return;
		}
		// accept only frame and frameset within frameset
		if (fSeenFramesetElement && elementCode != ExtendedHtmlElements.FRAME && elementCode != ExtendedHtmlElements.FRAMESET && elementCode != ExtendedHtmlElements.NOFRAMES) {
			notifyDiscardedStartElement(elem, attrs, augs);
			return;
		}

		if (elementCode == ExtendedHtmlElements.HEAD) {
			if (fSeenHeadElement) {
				notifyDiscardedStartElement(elem, attrs, augs);
				return;
			}
			fSeenHeadElement = true;
		} else if (elementCode == ExtendedHtmlElements.FRAMESET) {
		...
		} else if (elementCode == ExtendedHtmlElements.BODY) {
			// create <head></head> if none was present
			if (!fSeenHeadElement) {
				final QName head = createQName("head");
				forceStartElement(head, null, synthesizedAugs());
		...
		} else if (elementCode == ExtendedHtmlElements.FORM) {
			if (fOpenedForm) {
				notifyDiscardedStartElement(elem, attrs, augs);
				return;
			}
			fOpenedForm = true;
		} else if (elementCode == ExtendedHtmlElements.UNKNOWN) {
			consumeBufferedEndElements();
		}

		// check proper parent
		if (element.parent != null) {
			final ExtendedHtmlElements.Element preferedParent = element.parent[0];
			if (fDocumentFragment && (preferedParent.code == ExtendedHtmlElements.HEAD || preferedParent.code == ExtendedHtmlElements.BODY)) {
				...
				if (preferedParent.code != ExtendedHtmlElements.HEAD || (!fSeenBodyElement && !fDocumentFragment)) {
		...
		if ((fElementStack.top > 1
				&& (fElementStack.peek().element.code == ExtendedHtmlElements.SCRIPT))
				|| fElementStack.top > 2 && fElementStack.data[fElementStack.top - 2].element.code == ExtendedHtmlElements.HEAD) {

			//Skips close of special tags in head section
			if (ArrayUtils.contains(RECOGNIZED_HEAD_SPECIALS, fElementStack.data[fElementStack.top - 1].element.code)){
			} else {
		...
		if (elementCode == ExtendedHtmlElements.BODY) {
			lostText_.refeed(this);
		}
	...
	public void emptyElement(final QName element, XMLAttributes attrs, Augmentations augs)
		...
		final ExtendedHtmlElements.Element elem = getElement(element);
		if (elem.isEmpty()
				|| fAllowSelfclosingTags
				|| elem.code == ExtendedHtmlElements.UNKNOWN
				|| (elem.code == ExtendedHtmlElements.IFRAME && fAllowSelfclosingIframe)) {
	...
	public void startGeneralEntity(String name,
		...
		if (!fDocumentFragment) {
			...
			if (!insertBody) {
				...
				if (info.element.code == ExtendedHtmlElements.HEAD ||
						info.element.code == ExtendedHtmlElements.HTML) {
	...
	public void endElement(final QName element, final Augmentations augs) throws XNIException {
		...
		// get element information
		ExtendedHtmlElements.Element elem = getElement(element);

		// if we consider outside content, just buffer </body> and </html> to consider them at the very end
		if (!fIgnoreOutsideContent &&
				(elem.code == ExtendedHtmlElements.BODY || elem.code == ExtendedHtmlElements.HTML)) {
			endElementsBuffer_.add(new ElementEntry(element, augs));
			return;
		}

		// accept only frame and frameset within frameset
		if (fSeenFramesetElement && elem.code != ExtendedHtmlElements.FRAME && elem.code != ExtendedHtmlElements.FRAMESET) {
			notifyDiscardedEndElement(element, augs);
			return;
		}

		// check for end of document
		if (elem.code == ExtendedHtmlElements.HTML) {
			fSeenRootElementEnd = true;
		} else if (fIgnoreOutsideContent) {
			if (elem.code == ExtendedHtmlElements.BODY) {
				fSeenBodyElementEnd = true;
			} else if (fSeenBodyElementEnd) {
				notifyDiscardedEndElement(element, augs);
				return;
			}
		} else if (elem.code == ExtendedHtmlElements.FORM) {
			fOpenedForm = false;
		} else if (elem.code == ExtendedHtmlElements.HEAD && !forcedEndElement) {
		...
		if (depth == -1) {
			if (elem.code == ExtendedHtmlElements.P) {
		...
		if (depth > 1 && elem.isInline()) {
			...
			for (int i = 0; i < depth - 1; i++) {
				...
				final ExtendedHtmlElements.Element pelem = info.element;
				if (pelem.isInline() || pelem.code == ExtendedHtmlElements.FONT) { // TODO: investigate if only FONT
	 ...
	 */
	protected ExtendedHtmlElements.Element getElement(final QName elementName) {
		...
		return ExtendedHtmlElements.getElement(name);
	 ...
	 */
	protected final int getElementDepth(ExtendedHtmlElements.Element element) {
		final boolean container = element.isContainer();
		final short elementCode = element.code;
		final boolean tableBodyOrHtml = (elementCode == ExtendedHtmlElements.TABLE)
				|| (elementCode == ExtendedHtmlElements.BODY) || (elementCode == ExtendedHtmlElements.HTML);
		int depth = -1;
		for (int i = fElementStack.top - 1; i >= fragmentContextStackSize_; i--) {
			...
			if (info.element.code == ExtendedHtmlElements.TABLE && !tableBodyOrHtml) {
				return -1; // current element not allowed to close a table
			}
		}
		return depth;
	} // getElementDepth(ExtendedHtmlElements.Element)
	 ...
	 */
	protected int getParentDepth(ExtendedHtmlElements.Element[] parents, short bounds) {
		if (parents != null) {
		...
		return -1;
	} // getParentDepth(ExtendedHtmlElements.Element[],short):int
	...
	public static class Info {
		 ...
		 * The element.
		 */
		public ExtendedHtmlElements.Element element;
		 ...
		 */
		public Info(ExtendedHtmlElements.Element element, QName qname) {
			this(element, qname, null);
		} // <init>(ExtendedHtmlElements.Element,QName)
		 ...
		 */
		public Info(ExtendedHtmlElements.Element element,
		...
		} // <init>(ExtendedHtmlElements.Element,QName,XMLAttributes)
